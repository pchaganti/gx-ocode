...
# Import the tools
from ocode_python.tools.glob_tool import GlobTool, AdvancedGlobTool
...
from ocode_python.tools.notebook_tools import NotebookReadTool, NotebookEditTool
...
class TestThinkTool:
    ...
    async def test_think_tool_analyze(self):
        ...
        result = await tool.execute(
            ...
            topic="Code optimization strategies"
        )
        assert result.success
...
class TestArchitectTool:
    ...
    async def test_architect_tool_overview(self):
        ...
        assert result.success
        assert "Project Overview" in result.output or "overview" in result.output.lower()
...
class TestToolDefinitions:
    ...
    def test_all_tools_have_definitions(self):
        ...
        tools = [
            GlobTool(), AdvancedGlobTool(), GrepTool(), CodeGrepTool(),
            LsTool(), FileEditTool(), BashTool(), ScriptTool(),
            NotebookReadTool(), NotebookEditTool(), MemoryReadTool(), MemoryWriteTool(),
            ThinkTool(), ArchitectTool(), AgentTool(), MCPTool(), StickerRequestTool()
        ]
