...
class TestRunner:
    ...
    def run_all_tests(self):
        ...
        # Test FileEditTool
        self.run_test(self.test_file_edit_tool_basic, "FileEditTool basic functionality")
    ...
    async def test_ls_tool_basic(self):
        ...
        assert "Directory listing" in result.output or len(result.output) > 0, "Should produce output"
    ...
    async def test_file_edit_tool_basic(self):
        ...
        try:
            ...
            with open(temp_path, 'r') as f:
                content = f.read()
                assert "Hello world" in content and "Test line" in content, "Content should be appended"
        finally:
    ...
    async def test_think_tool_basic(self):
        ...
        tool = ThinkTool()
        result = await tool.execute(
            thinking_type="analyze",
            topic="Test topic"
        )
        assert result.success, "ThinkTool should succeed with analyze type"
        assert "Analysis" in result.output or "analysis" in result.output.lower(), "Should produce analysis output"
    ...
    def test_tool_definitions(self):
        ...
        tools = [
            GlobTool(), GrepTool(), LsTool(), FileEditTool(),
            BashTool(), ThinkTool(), AgentTool(), StickerRequestTool()
        ]
        
        for tool in tools:
            definition = tool.definition
            assert definition.name, f"{tool.__class__.__name__} should have a name"
            assert definition.description, f"{tool.__class__.__name__} should have a description"
            assert isinstance(definition.parameters, list), f"{tool.__class__.__name__} should have parameters list"
