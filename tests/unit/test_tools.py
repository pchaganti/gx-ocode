...
from ocode_python.tools.base import (
    ToolRegistry, Tool, ToolDefinition, ToolParameter, ToolResult
)
from ocode_python.tools.file_tools import FileReadTool, FileWriteTool, FileListTool
from ocode_python.tools.git_tools import GitStatusTool, GitCommitTool
...
class TestToolRegistry:
    ...
    async def test_all_tools_registration_and_execution(self, temp_dir: Path):
        ...
        for tool in registry.get_all_tools():
            ...
            if tool_name in ["git_status", "git_commit", "git_diff", "test_runner", "mcp"]:
                continue
            ...
            try:
                ...
                if tool_name == "file_read":
                    ...
                    params = {"path": str(test_file), "operation": "append", "content": "test"}
                elif tool_name == "bash":
                    params = {"script": "echo test"}
                elif tool_name == "memory_read":
                    params = {"memory_type": "persistent"}
                elif tool_name == "memory_write":
                    params = {"memory_type": "persistent", "key": "test", "value": "test"}
                ...
                # Basic validation
                assert isinstance(result, ToolResult), f"Tool {tool_name} did not return ToolResult"
                assert hasattr(result, "success"), f"Tool {tool_name} result missing success attribute"
                assert hasattr(result, "output"), f"Tool {tool_name} result missing output attribute"
    ...
    async def test_tool_name_mapping(self):
        ...
        for input_name, expected_name in test_cases:
            ...
            assert mapped_name == expected_name, f"Tool name mapping failed: {input_name} -> {mapped_name} (expected {expected_name})"
            
            # Verify the mapped name exists in the registry
            tool = registry.get_tool(mapped_name)
            assert tool is not None, f"Mapped tool name {mapped_name} not found in registry"
