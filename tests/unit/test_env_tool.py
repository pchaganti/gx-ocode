...
class TestEnvironmentTool:
    ...
    async def test_set_action_update_existing(self, tool):
        ...
        with patch.dict(os.environ, {'EXISTING_VAR': 'old_value'}, clear=True):
            result = await tool.execute(action="set", name="EXISTING_VAR", value="new_value")
    ...
    async def test_save_action(self, tool):
        ...
        try:
            with patch.dict(os.environ, test_env, clear=True):
                result = await tool.execute(action="save", file=temp_path, pattern="SAVE")
        ...
        try:
            with patch.dict(os.environ, test_env, clear=True):
                ...
                with open(temp_path, 'r') as f:
                    ...
                    assert 'VAR_WITH_EQUALS=key=value' in content or 'VAR_WITH_EQUALS="key=value"' in content
        finally:
            try:
                os.unlink(temp_path)
            except:
    ...
    async def test_invalid_action(self, tool):
        ...
        assert not result.success
        assert "Invalid action" in result.error and "get, set, unset, list, load, save" in result.error
