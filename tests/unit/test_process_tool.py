...
class TestProcessMonitorTool:
    ...
    async def test_list_action(self, tool):
        ...
        process1.cpu_percent.return_value = 25.5
    ...
    async def test_list_action_sorted(self, tool):
        ...
        low_cpu.cpu_percent.return_value = 25.5
        ...
        mock_iter.return_value = [low_cpu, high_cpu_process]
            ...
            cpu_intensive_index = next(i for i, line in enumerate(lines) if "cpu_intensive" in line)
            test_process_index = next(i for i, line in enumerate(lines) if "test_process" in line)
    ...
    async def test_list_action_with_limit(self, tool):
        ...
        for i in range(20):
            ...
            process.cpu_percent.return_value = i * 2.0
    ...
    async def test_find_action(self, tool):
        ...
        chrome_process.exe.return_value = "/Applications/Chrome.app/Contents/MacOS/chrome"
    ...
    async def test_info_action(self, tool):
        ...
        with patch('psutil.Process') as mock_process_class:
            ...
            assert "Command:" in result.output and "/usr/bin/test_process --arg1 value1" in result.output
    ...
    async def test_invalid_action(self, tool):
        ...
        assert not result.success
        assert "Invalid action" in result.error and "list, find, info, check" in result.error
