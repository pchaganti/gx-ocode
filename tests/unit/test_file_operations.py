...
class TestAsyncFileOperations:
    ...
    async def test_async_operations_with_retry(self):
        ...
        with pytest.raises(FileSystemError):
            await safe_file_read_async("/nonexistent/file.txt", retry_config=retry_config)


class TestFileUtilities:
    """Test file utility functions."""

    @pytest.mark.skipif(os.name != 'nt', reason="File locking check is Windows-specific")
...
class TestRetryIntegration:
    ...
    def test_retry_on_transient_failure(self):
        ...
        try:
            ...
            def mock_open(*args, **kwargs):
                nonlocal call_count
                call_count += 1
                if call_count == 1 and 'w' in str(kwargs.get('mode', args[1] if len(args) > 1 else '')):
    ...
    def test_exhaust_all_retries(self):
        ...
        with pytest.raises(FileSystemError):
            safe_file_read("/definitely/nonexistent/file.txt", retry_config=retry_config)
...
class TestErrorHandling:
    ...
    def test_structured_error_context(self):
        ...
        try:
            ...
            assert "/nonexistent/file.txt" in str(e.context.details.get("file_path", ""))
