...
from ocode_python.utils.security import (
    PermissionManager, PermissionRule, PermissionLevel, OperationType,
    SecureShellExecutor
)
...
class TestPermissionManager:
    ...
    def test_load_default_rules(self):
        ...
        write_rules = [r for r in manager.rules if r.operation == OperationType.FILE_WRITE]
        assert len(write_rules) > 0
        
        exec_rules = [r for r in manager.rules if r.operation == OperationType.SHELL_EXEC]
    ...
    def test_check_permission_file_read(self):
        ...
        permission = manager.check_permission(OperationType.FILE_READ, "/home/user/document.txt")
    ...
    def test_validate_file_operation_read(self, temp_dir: Path):
        ...
        allowed, reason = manager.validate_file_operation("read", str(temp_dir / "nonexistent.txt"))
        assert not allowed
    ...
    def test_validate_file_operation_write(self, temp_dir: Path):
        ...
        allowed, reason = manager.validate_file_operation("write", "/etc/malicious.conf")
        assert not allowed
    ...
    def test_custom_validator(self):
        ...
        permission = manager.check_permission(OperationType.FILE_READ, "/home/user/normal.txt")
        assert permission != PermissionLevel.DENIED
        
        # File with "secret" should be blocked
        permission = manager.check_permission(OperationType.FILE_READ, "/home/user/secret_file.txt")
        assert permission == PermissionLevel.DENIED
