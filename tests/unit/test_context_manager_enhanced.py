...
from ocode_python.core.context_manager import (
    ContextManager, FileInfo, ProjectContext
)
...
class TestContextManagerEnhanced:
    ...
    def test_detect_multi_action_query(self):
        ...
        result = manager._detect_multi_action_query("run tests and then commit if they pass")
        ...
        result = manager._detect_multi_action_query("find all TODO comments and replace them")
        assert result is not None
        assert result['multi_action'] is True
    ...
    async def test_build_context_validation(self):
        ...
        context = await manager.build_context("test", max_files=5000)
    ...
    async def test_concurrent_file_analysis(self, temp_dir: Path):
        ...
        with patch.object(manager, 'analyze_file', side_effect=track_concurrent_analyze):
            context = await manager.build_context("test", max_files=15)
...
class TestProjectContextEnhanced:
    ...
    def test_get_relevant_files_scoring(self, temp_dir: Path):
        ...
        context = ProjectContext(
            ...
            file_info={
                temp_dir / "auth" / "login.py": FileInfo(
