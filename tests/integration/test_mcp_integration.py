...
from ocode_python.mcp.protocol import MCPProtocol, MCPServer, JSONRPCRequest, JSONRPCResponse
...
class TestOCodeMCPServerIntegration:
    ...
    async def test_ocode_special_resources(self, mock_project_dir):
        ...
        structure_result = await server._read_ocode_resource("ocode://project/structure")
    ...
    async def test_full_mcp_workflow(self, mock_project_dir):
        ...
        tool_result = await server._handle_call_tool({
            "name": "file_list",
            "arguments": {"path": str(mock_project_dir)}
        })
