...
class TestOCodeEngineIntegration:
    ...
    @pytest.mark.asyncio
    async def test_engine_context_building(self, ocode_engine: OCodeEngine, mock_project_dir: Path):
    ...
    def test_engine_clear_context(self, ocode_engine: OCodeEngine):
        ...
        ocode_engine.current_context = ProjectContext(
            files={}, file_info={}, dependencies={}, symbols={}, project_root=Path("/test")
        )
...
class TestEngineWithRealFiles:
    ...
    async def test_engine_file_analysis(self, mock_project_dir: Path):
        ...
        async def mock_stream(*args, **kwargs):
            chunks = [
                StreamChunk(content="I can see this is a Python project with several modules."),
                StreamChunk(done=True)
            ]
            for chunk in chunks:
                yield chunk
        ...
        # Should have found Python files
        python_files = [f for f in engine.current_context.files.keys() if f.suffix == '.py']
    ...
    async def test_engine_with_git_info(self, mock_git_repo: Path):
        ...
        async def mock_stream(*args, **kwargs):
            chunks = [
                ...
                StreamChunk(content="I can see the current branch and commit information."),
                StreamChunk(done=True)
            ]
            for chunk in chunks:
                yield chunk
...
class TestEngineSessionManagement:
    ...
    @pytest.mark.asyncio
    async def test_save_and_load_session(self, ocode_engine: OCodeEngine, temp_dir: Path):
    ...
    async def test_session_persistence(self, ocode_engine: OCodeEngine):
        ...
        assert len(ocode_engine.conversation_history) == len(queries) * 2  # User + assistant each
