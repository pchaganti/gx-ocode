    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            for match in matches:
                ...
                if not include_hidden and any(part.startswith('.') for part in match_path.parts):
                    continue
            ...
            if not filtered_matches:
                ...
                output_lines = [f"Found {len(filtered_matches)} matches for pattern '{pattern}':"]
                ...
                if len(matches) > max_results:
                    output_lines.append(f"  ... and {len(matches) - max_results} more (use max_results to see more)")
            ...
            return ToolResult(
                ...
                metadata={
                    ...
                    "files": [str(Path(m).relative_to(base_path)) for m in filtered_matches]
                }
            )
...
class AdvancedGlobTool(GlobTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        for file_rel_path in result.metadata.get("files", []):
            ...
            if exclude_patterns:
                if any(fnmatch.fnmatch(str(file_path), exclude_pat) for exclude_pat in exclude_patterns):
                    continue
            ...
            if modified_since:
                try:
                    ...
                    since_time = datetime.fromisoformat(modified_since.replace('Z', '+00:00'))
