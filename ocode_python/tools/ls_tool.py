...
class LsTool(Tool):
        ...
        try:
            ...
            if recursive:
                files_info = self._list_recursive(
                    target_path, all, long_format, max_depth, 
                    file_types, extensions, size_filter
                )
            else:
                files_info = self._list_directory(
                    target_path, all, long_format,
                    file_types, extensions, size_filter
                )
    ...
    def _list_directory(self, dir_path: Path, show_hidden: bool, long_format: bool,
                       file_types: List[str], extensions: List[str],
                       size_filter: Optional[Dict[str, str]]) -> List[Dict[str, Any]]:
    ...
    def _list_recursive(self, base_path: Path, show_hidden: bool, long_format: bool,
                       max_depth: int, file_types: List[str], extensions: List[str], 
                       size_filter: Optional[Dict[str, str]]) -> List[Dict[str, Any]]:
    ...
    def _passes_filters(self, file_info: Dict[str, Any], file_types: List[str],
                       extensions: List[str], size_filter: Optional[Dict[str, str]]) -> bool:
    ...
    def _sort_files(self, files_info: List[Dict[str, Any]], sort_by: str, reverse: bool) -> List[Dict[str, Any]]:
    ...
    def _format_listing(self, files_info: List[Dict[str, Any]], long_format: bool, base_path: Path) -> str:
        ...
        if long_format:
            ...
            # Header
            lines.append(f"{'Permissions':<11} {'Size':>{max_size_width}} {'Modified':<19} {'Name'}")
            ...
            for file_info in files_info:
                ...
                lines.append(f"{perms:<11} {size:>{max_size_width}} {modified:<19} {name}")
        else:
            # Simple format
            for file_info in files_info:
                name = file_info["name"]
        ...
        lines.append("")
        lines.append(f"Total: {file_count} files, {dir_count} directories, {self._format_size(total_size)}")
        ...
        def _render_tree(tree_dict: Dict, prefix: str = "", is_last: bool = True):
            ...
            for i, (name, content) in enumerate(items):
                ...
                if isinstance(content, dict) and not content.get("type"):
                    ...
                    icon = {"dir": "📁", "file": "📄", "executable": "⚡", "link": "🔗"}.get(file_type, "📄")
    ...
    def _format_size(self, size: int) -> str:
        ...
        for unit in ['B', 'KB', 'MB', 'GB', 'TB']:
            if size_float < 1024.0:
                return f"{size_float:.1f}{unit}" if size_float != int(size_float) else f"{int(size_float)}{unit}"
