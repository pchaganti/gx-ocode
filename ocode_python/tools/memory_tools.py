...
class MemoryReadTool(Tool):
        ...
        try:
            ...
            if not memory_type:
                return ToolResult(
                    success=False,
                    output="",
                    error="memory_type parameter is required"
                )
            ...
            if memory_type == "session" or memory_type == "all":
                session_data = await self._read_session_memory(memory_dir, session_id, key, category)
                result["session"] = session_data
                if memory_type != "all":
                    result["entries"] = session_data.get("entries", [])
            
            if memory_type == "context" or memory_type == "all":
                context_data = await self._read_context_memory(memory_dir, key, category)
                result["context"] = context_data
                if memory_type != "all":
                    result["entries"] = context_data.get("entries", [])
            
            if memory_type == "persistent" or memory_type == "all":
                persistent_data = await self._read_persistent_memory(memory_dir, key, category)
            ...
            if memory_type == "all":
                ...
                if not all_entries and (key or category):
                    ...
                    if "session" in result:
                        session_data = await self._read_session_memory(memory_dir, session_id, None, None)
                        all_entries.extend(session_data.get("entries", []))
                    if "context" in result:
                        context_data = await self._read_context_memory(memory_dir, None, None)
                        all_entries.extend(context_data.get("entries", []))
                    if "persistent" in result:
                        persistent_data = await self._read_persistent_memory(memory_dir, None, None)
            ...
            return ToolResult(
                success=False,
                output="",
                error=f"Error reading memory: {str(e)}"
            )
    
    def _get_memory_dir(self) -> Path:
        """Get the memory directory path."""
        # Always use home directory for consistency across CLI and REPL
        memory_dir = Path.home() / ".ocode" / "memory"
        memory_dir.mkdir(parents=True, exist_ok=True)
        return memory_dir
    
    async def _read_session_memory(self, memory_dir: Path, session_id: Optional[str] = None,
                                  key: Optional[str] = None, category: Optional[str] = None) -> Dict[str, Any]:
        ...
        if session_id:
            ...
            if session_file.exists():
                ...
                result["entries"] = self._filter_entries(session_data.get("data", {}), key, category)
        else:
            ...
            for session_file in session_files:
                try:
                    ...
                    entries = self._filter_entries(session_data.get("data", {}), key, category)
    ...
    async def _read_context_memory(self, memory_dir: Path, key: Optional[str] = None,
    ...
    async def _read_persistent_memory(self, memory_dir: Path, key: Optional[str] = None,
                                     category: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _filter_entries(self, data: Dict[str, Any], key: Optional[str] = None,
...
class MemoryWriteTool(Tool):
    ...
    async def _write_session_memory(self, memory_dir: Path, operation: str,
                                   key: Optional[str] = None, value: Optional[Any] = None,
                                   category: Optional[str] = None, session_id: Optional[str] = None,
        ...
        result = self._perform_operation(session_data["data"], operation, key, value, category, metadata)
    ...
    async def _write_context_memory(self, memory_dir: Path, operation: str,
                                   key: Optional[str] = None, value: Optional[Any] = None,
                                   category: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        result = self._perform_operation(context_data, operation, key, value, category, metadata)
    ...
    async def _write_persistent_memory(self, memory_dir: Path, operation: str,
                                      key: Optional[str] = None, value: Optional[Any] = None,
                                      category: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        result = self._perform_operation(persistent_data, operation, key, value, category, metadata)
    ...
    def _perform_operation(self, data: Dict[str, Any], operation: str,
                          key: Optional[str] = None, value: Optional[Any] = None,
                          category: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        if operation == "set":
            ...
            current_value = data[key].get("value", []) if isinstance(data[key], dict) else data[key]
