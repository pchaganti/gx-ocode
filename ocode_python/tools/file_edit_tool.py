...
class FileEditTool(Tool):
        ...
        try:
            ...
            if operation == "replace":
                result = await self._replace_operation(
                    ...
                    regex, case_sensitive, whole_word, max_replacements
                )
                ...
                result = await self._delete_operation(
                    original_lines, search_pattern, line_number, line_range,
            ...
            if dry_run:
                ...
                if new_content != original_content:
                    output_lines.append("\\nPreview of changes:")
                    output_lines.append(self._create_diff_preview(original_content, new_content))
                else:
            ...
            return ToolResult(
                ...
                metadata={
                    ...
                    "dry_run": dry_run,
                    "backup_created": backup and not dry_run and new_content != original_content,
            ...
            return ToolResult(
                success=False,
                output="",
                error=f"File edit operation failed: {str(e)}"
            )

    async def _replace_operation(self, content: str, search_pattern: Optional[str],
                               replacement: Optional[str], regex: bool, case_sensitive: bool,
                               whole_word: bool, max_replacements: int) -> Dict[str, Any]:
        """Perform find and replace operation."""
        if not search_pattern:
            return {"success": False, "error": "search_pattern is required for replace operation"}
    ...
    async def _insert_operation(self, lines: List[str], content: Optional[str],
                              line_number: Optional[int]) -> Dict[str, Any]:
        """Insert content at specified line."""
        if content is None:
            return {"success": False, "error": "content is required for insert operation"}
        
        if line_number is None:
            return {"success": False, "error": "line_number is required for insert operation"}
    ...
    async def _delete_operation(self, lines: List[str], search_pattern: Optional[str],
                              line_number: Optional[int], line_range: Optional[Dict[str, int]],
                              regex: bool, case_sensitive: bool, whole_word: bool) -> Dict[str, Any]:
        ...
        if line_number is not None:
            ...
            try:
                ...
                changes = [f"Deleted {deleted_count} line(s) matching pattern '{search_pattern}'"]

            except re.error as e:
                return {"success": False, "error": f"Invalid regex pattern: {str(e)}"}
        else:
            return {"success": False, "error": "line_number, line_range, or search_pattern is required for delete operation"}
    ...
    async def _append_operation(self, content: str, new_content: Optional[str]) -> Dict[str, Any]:
    ...
    async def _prepend_operation(self, content: str, new_content: Optional[str]) -> Dict[str, Any]:
        """Prepend content to beginning of file."""
        if new_content is None:
            return {"success": False, "error": "content is required for prepend operation"}
    ...
    def _create_diff_preview(self, original: str, new: str) -> str:
        ...
        if len(new_lines) > len(original_lines):
            for i in range(len(original_lines), min(len(new_lines), len(original_lines) + max_lines)):
                diff_lines.append(f"  {i+1:3d}+ {new_lines[i]}")
        elif len(original_lines) > len(new_lines):
            for i in range(len(new_lines), min(len(original_lines), len(new_lines) + max_lines)):
