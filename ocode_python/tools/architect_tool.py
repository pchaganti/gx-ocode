...
class ArchitectTool(Tool):
        ...
        return ToolDefinition(
            ...
            parameters=[
                ...
                ToolParameter(
                    ...
                    required=False,
                    default=["*test*", "*__pycache__*", "node_modules", ".git", "venv", ".env"]
                ),
                ToolParameter(
                    name="output_format",
                    type="string",
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            if exclude_patterns is None:
                exclude_patterns = ["*test*", "*__pycache__*", "node_modules", ".git", "venv", ".env"]
            ...
            if analysis_type == "overview":
                results = await self._analyze_overview(analysis_path, language, include_patterns, exclude_patterns)
            elif analysis_type == "dependencies":
                results = await self._analyze_dependencies(analysis_path, language, depth, include_patterns, exclude_patterns)
            elif analysis_type == "structure":
                results = await self._analyze_structure(analysis_path, language, depth, include_patterns, exclude_patterns)
            elif analysis_type == "patterns":
                results = await self._analyze_patterns(analysis_path, language, include_patterns, exclude_patterns)
            elif analysis_type == "metrics":
                results = await self._analyze_metrics(analysis_path, language, include_patterns, exclude_patterns)
            elif analysis_type == "health":
                results = await self._analyze_health(analysis_path, language, include_patterns, exclude_patterns)
            elif analysis_type == "diagram":
                results = await self._generate_diagram(analysis_path, language, include_patterns, exclude_patterns)
            else:
                return ToolResult(
                    success=False,
                    output="",
    ...
    def _should_include_file(self, file_path: Path, include_patterns: Optional[List[str]] = None,
        ...
        if include_patterns:
            return any(self._matches_pattern(file_str, pattern) for pattern in include_patterns)
        
        # Default: include common source code files
        return file_path.suffix.lower() in [".py", ".js", ".jsx", ".ts", ".tsx", ".java", ".go", ".rs", ".cpp", ".c", ".h"]
    ...
    async def _analyze_overview(self, path: Path, language: str, include_patterns: Optional[List[str]],
                               exclude_patterns: Optional[List[str]]) -> Dict[str, Any]:
        ...
        if path.is_file():
            ...
            # Analyze directory
            for file_path in path.rglob("*"):
                if file_path.is_file() and self._should_include_file(file_path, include_patterns, exclude_patterns):
                    total_files += 1
    ...
    def _analyze_javascript_file(self, content: str) -> Dict[str, Any]:
        ...
        function_pattern = re.compile(r'function\s+(\w+)|(\w+)\s*[=:]\s*function|(\w+)\s*[=:]\s*\([^)]*\)\s*=>')
    ...
    def _classify_architecture_file(self, filename: str) -> str:
        ...
        elif filename_lower in ["package.json", "pyproject.toml", "setup.py", "requirements.txt"]:
            return "dependencies"
        elif filename_lower in ["dockerfile", "docker-compose.yml", "docker-compose.yaml"]:
            return "containerization"
        elif filename_lower in ["makefile"]:
            return "build"
        elif filename_lower.startswith(".env") or "config" in filename_lower or "settings" in filename_lower:
    ...
    async def _analyze_dependencies(self, path: Path, language: str, depth: int,
                                   include_patterns: Optional[List[str]], exclude_patterns: Optional[List[str]]) -> Dict[str, Any]:
        ...
        if path.is_file():
            ...
            files_to_analyze = [f for f in path.rglob("*") 
                              if f.is_file() and self._should_include_file(f, include_patterns, exclude_patterns)]
        ...
        dependencies["circular_dependencies"] = self._detect_circular_dependencies(dependencies["internal"])
        
        # Calculate statistics
        dependencies["statistics"] = {
            "files_analyzed": len(files_analyzed),
            "total_internal_deps": sum(len(deps) for deps in dependencies["internal"].values()),
            "total_external_deps": len(dependencies["external"]),
            "avg_deps_per_file": sum(len(deps) for deps in dependencies["internal"].values()) / max(len(files_analyzed), 1),
            "files_with_no_deps": len([f for f in files_analyzed if f not in dependencies["internal"] or not dependencies["internal"][f]]),
            "most_dependent_files": sorted(dependencies["internal"].items(), key=lambda x: len(x[1]), reverse=True)[:5]
        }
        
        return dependencies
    
    def _extract_file_dependencies(self, file_path: Path, language: str) -> Dict[str, List[str]]:
        ...
        try:
            ...
            elif language in ["javascript", "typescript"] or file_path.suffix in [".js", ".jsx", ".ts", ".tsx"]:
    ...
    def _extract_python_dependencies(self, content: str, file_path: Path) -> Dict[str, List[str]]:
    ...
    def _extract_javascript_dependencies(self, content: str, file_path: Path) -> Dict[str, List[str]]:
    ...
    def _find_project_root(self, file_path: Path) -> Optional[Path]:
        ...
        while current != current.parent:
            ...
            if any((current / indicator).exists() for indicator in 
                   ['setup.py', 'pyproject.toml', 'package.json', '.git', 'Makefile']):
                return current
    ...
    async def _analyze_structure(self, path: Path, language: str, depth: int,
                                include_patterns: Optional[List[str]], exclude_patterns: Optional[List[str]]) -> Dict[str, Any]:
        ...
        if path.is_file():
            ...
            # Directory analysis
            structure["hierarchy"] = self._build_directory_tree(path, depth, include_patterns, exclude_patterns)
            
            for file_path in path.rglob("*"):
                if not file_path.is_file() or not self._should_include_file(file_path, include_patterns, exclude_patterns):
                    continue
        ...
        structure["metrics"] = {
            ...
            "avg_files_per_directory": self._calculate_avg_files_per_dir(structure["hierarchy"])
        }
        
        return structure
    
    def _build_directory_tree(self, path: Path, max_depth: int, include_patterns: Optional[List[str]],
                             exclude_patterns: Optional[List[str]], current_depth: int = 0) -> Dict[str, Any]:
        ...
        try:
            for item in path.iterdir():
                ...
                if item.is_dir():
                    if not any(self._matches_pattern(str(item), pattern) for pattern in (exclude_patterns or [])):
                        tree["children"][item.name] = self._build_directory_tree(
                            item, max_depth, include_patterns, exclude_patterns, current_depth + 1
                        )
                elif item.is_file() and self._should_include_file(item, include_patterns, exclude_patterns):
                    tree["files"].append({
                        "name": item.name,
    ...
    def _is_test_file(self, file_path: Path) -> bool:
        ...
        return any(pattern in name_lower for pattern in ['test', 'spec', '_test', '.test'])
    
    def _is_documentation_file(self, file_path: Path) -> bool:
        """Check if a file is documentation."""
        name_lower = file_path.name.lower()
        return any(pattern in name_lower for pattern in ['readme', 'doc', 'docs', '.md', '.txt']) or \
               file_path.suffix.lower() in ['.md', '.txt', '.rst']
    
    def _is_configuration_file(self, file_path: Path) -> bool:
        """Check if a file is a configuration file."""
        name_lower = file_path.name.lower()
        return any(pattern in name_lower for pattern in ['config', 'settings', '.env', 'setup']) or \
    ...
    def _is_entry_point(self, file_path: Path) -> bool:
        ...
        name_lower = file_path.name.lower()
        return name_lower in ['main.py', 'app.py', 'index.js', 'index.ts', 'server.js', 'run.py']
    ...
    async def _analyze_patterns(self, path: Path, language: str, include_patterns: Optional[List[str]],
                               exclude_patterns: Optional[List[str]]) -> Dict[str, Any]:
    ...
    async def _analyze_metrics(self, path: Path, language: str, include_patterns: Optional[List[str]],
                              exclude_patterns: Optional[List[str]]) -> Dict[str, Any]:
    ...
    async def _analyze_health(self, path: Path, language: str, include_patterns: Optional[List[str]],
                             exclude_patterns: Optional[List[str]]) -> Dict[str, Any]:
        ...
        structure = await self._analyze_structure(path, language, 2, include_patterns, exclude_patterns)
        metrics = await self._analyze_metrics(path, language, include_patterns, exclude_patterns)
        dependencies = await self._analyze_dependencies(path, language, 2, include_patterns, exclude_patterns)
        ...
        total_files = complexity_dist["low"] + complexity_dist["medium"] + complexity_dist["high"]
        if total_files > 0:
        ...
        if scores.get("complexity", 0) < 70:
            ...
            health["recommendations"].append("Consider refactoring complex functions and classes")
        
        if scores.get("dependencies", 0) < 70:
            health["issues"].append("Circular dependencies detected")
            health["recommendations"].append("Resolve circular dependencies to improve maintainability")
        ...
        if scores.get("testing", 0) < 70:
            ...
            health["recommendations"].append("Add more unit tests and integration tests")
        
        return health
    
    async def _generate_diagram(self, path: Path, language: str, include_patterns: Optional[List[str]],
                               exclude_patterns: Optional[List[str]]) -> Dict[str, Any]:
    ...
    def _format_summary_output(self, results: Dict[str, Any], analysis_type: str) -> str:
        ...
        if analysis_type == "overview":
            ...
            output += f"• Average File Size: {summary.get('avg_file_size', 0)} lines\n\n"
            ...
            output += f"• Internal Dependencies: {stats.get('total_internal_deps', 0)}\n"
            output += f"• External Dependencies: {stats.get('total_external_deps', 0)}\n"
            ...
            output += f"Codebase Health Score: {results.get('overall_score', 0):.1f}/100\n\n"
    ...
    def _format_detailed_output(self, results: Dict[str, Any], analysis_type: str) -> str:
    ...
    def _format_mermaid_output(self, results: Dict[str, Any], analysis_type: str) -> str:
