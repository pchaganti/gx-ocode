...
class EnvironmentTool(Tool):
    ...
    async def _action_get(self, name: str, output_format: str) -> ToolResult:
        ...
        if not self._is_valid_var_name(name):
            return ToolResult(
                success=False,
                output="",
                error=f"Invalid variable name: {name}"
            )
    ...
    async def _action_list(self, pattern: Optional[str], output_format: str) -> ToolResult:
    ...
    async def _action_load(self, file_path: str, export: bool, output_format: str) -> ToolResult:
