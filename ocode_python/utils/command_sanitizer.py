...
class CommandSanitizer:
    ...
    def sanitize_command(self, command: str, strict_mode: bool = True) -> Tuple[bool, str, Optional[str]]:
        """
    ...
    def _validate_rm(self, parts: List[str]) -> Tuple[bool, Optional[str]]:
        ...
        if "-r" in parts or "-rf" in parts or "-fr" in parts:
            for part in parts[1:]:
                ...
                if re.match(r"^/(bin|boot|dev|etc|lib|proc|root|sbin|sys|usr)/?$", part):
    ...
    def _validate_chmod(self, parts: List[str]) -> Tuple[bool, Optional[str]]:
        ...
        for part in parts[1:]:
            if part.startswith("-"):
                continue
            if part.startswith("/etc/") or part.startswith("/bin/") or part.startswith("/usr/bin/"):
        ...
        for part in parts[1:]:
            ...
            if re.match(r"^[4267]\d{3}$", part):  # First digit 4,2,6 means setuid/setgid
    ...
    def _validate_chown(self, parts: List[str]) -> Tuple[bool, Optional[str]]:
        ...
        for part in parts[1:]:
            if part.startswith("-"):
                continue
            if part.startswith("/etc/") or part.startswith("/bin/") or part.startswith("/usr/"):
...
def sanitize_command(command: str, strict_mode: bool = True) -> Tuple[bool, str, Optional[str]]:
