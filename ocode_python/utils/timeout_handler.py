...
class TimeoutError(Exception):
    ...
    def __init__(self, message: str, operation: Optional[str] = None, 
                 duration: Optional[float] = None, context: Optional[dict] = None):
...
def async_timeout(seconds: float, operation: Optional[str] = None, 
...
async def with_timeout(coro, timeout: float, operation: Optional[str] = None,
                      default: Optional[T] = None, raise_on_timeout: bool = True) -> T:
    """
...
class AdaptiveTimeout:
    ...
    def __init__(self, base_timeout: float, min_timeout: float = 1.0,
                 max_timeout: float = 300.0, adjustment_factor: float = 1.5):
    ...
    async def execute(self, coro, operation: Optional[str] = None):
        ...
        try:
            ...
            self.current_timeout = min(
                self.current_timeout * self.adjustment_factor,
                self.max_timeout
            )
            raise