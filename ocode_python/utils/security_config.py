...
class SecurityPatterns:
    ...
    def __post_init__(self) -> None:
        ...
        self._absolute_blocked_regex = [re.compile(pattern, re.IGNORECASE) for pattern in self.absolute_blocked]
        self._requires_confirmation_regex = [re.compile(pattern, re.IGNORECASE) for pattern in self.requires_confirmation]
        self._redirection_blocked_regex = [re.compile(pattern) for pattern in self.redirection_blocked]
        self._command_chaining_blocked_regex = [re.compile(pattern) for pattern in self.command_chaining_blocked]
        self._safe_pipe_exceptions_regex = [re.compile(pattern, re.IGNORECASE) for pattern in self.safe_pipe_exceptions]
...
class SecurityConfigManager:
    ...
    def __init__(self, config_path: Optional[Path] = None) -> None:
        ...
        if config_path is None:
            config_path = Path(__file__).parent.parent / "config" / "security_patterns.json"
    ...
    def _load_config(self) -> SecurityPatterns:
        ...
        try:
            ...
            return SecurityPatterns(
                ...
                command_chaining_blocked=patterns["command_chaining_blocked"]["patterns"],
            ...
            return SecurityPatterns(
                absolute_blocked=[
                    ...
                    "sudo rm", "sudo dd", "curl.*\\|\\s*bash", "wget.*\\|\\s*bash",
                    "\\$\\(", "`", "&&\\s*(rm|dd|mkfs)", "||\\s*(rm|dd|mkfs)"
                ],
                requires_confirmation=[
                    "\\|", "grep\\s", "sort\\s", "wc\\s", "curl\\s", "wget\\s",
                    "tar\\s", "chmod\\s", "chown\\s", "rm\\s", "mv\\s", "cp\\s.*-r"
                ],
                redirection_blocked=[">", ">>", "2>", "&>"],
                command_chaining_blocked=[";", "&&", "||"],
                safe_pipe_exceptions=[
                    "\\|\\s*wc\\s+-l", "\\|\\s*head\\s", "\\|\\s*tail\\s",
                    "\\|\\s*sort$", "\\|\\s*uniq$", "\\|\\s*cat$"
                ],
    ...
    def validate_command(self, command: str) -> Tuple[str, Optional[str], bool]:
        ...
        for regex in patterns._absolute_blocked_regex:
            if regex.search(command):
                return "blocked", f"Command contains prohibited pattern: {regex.pattern}", False
        
        # Check for redirection patterns
        for regex in patterns._redirection_blocked_regex:
            if regex.search(command):
                return "blocked", f"Output redirection not permitted: {regex.pattern}", False
        
        # Check for command chaining patterns  
        for regex in patterns._command_chaining_blocked_regex:
            if regex.search(command):
                return "blocked", f"Command chaining not permitted: {regex.pattern}", False
        
        # Check if command has pipes - see if it's a safe exception
        if "|" in command:
            is_safe_pipe = any(regex.search(command) for regex in patterns._safe_pipe_exceptions_regex)
            if is_safe_pipe:
        ...
        for regex in patterns._requires_confirmation_regex:
            if regex.search(command):
                return "requires_confirmation", f"Command matches sensitive pattern: {regex.pattern}", True
