...
class PathValidator:
        ...
        try:
            ...
            for char in path_str:
                if char in self.dangerous_chars:
                    return False, f"Path contains forbidden character: {repr(char)}", None
    ...
    def _has_path_traversal(self, path: str) -> bool:
        ...
        parts = path.replace(self.alt_separator, self.separator).split(self.separator) if self.alt_separator else path.split(self.separator)
    ...
    def sanitize_filename(self, filename: str) -> str:
        ...
        sanitized = "".join(c if c not in self.dangerous_chars else "_" for c in filename)
