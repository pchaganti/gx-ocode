...
class ContextManager:
    ...
    def __init__(self, root: Optional[Path] = None, cache_dir: Optional[Path] = None):
        ...
        self.wildcard_patterns = [
            pattern for pattern in self.ignore_patterns 
    ...
    def _manage_cache_size(self) -> None:
        ...
        if len(self.file_cache) > self.max_cache_size:
            ...
            sorted_entries = sorted(
                ...
                key=lambda x: x[1][1]  # Sort by mtime
            )
    ...
    def _detect_multi_action_query(self, query_lower: str) -> Optional[Dict[str, Any]]:
        ...
        for pattern_def in multi_patterns:
            if re.search(pattern_def['pattern'], query_lower):
                all_tools = pattern_def['primary_tools'] + pattern_def['secondary_tools']
        ...
        if has_conjunction:
            ...
            import re
            parts = re.split(r'\s+(and|then|after|followed by|next|also)\s+', query_lower)
    ...
    def _quick_categorize_part(self, part: str) -> Optional[Dict[str, Any]]:
        ...
        if any(kw in part_lower for kw in ['build', 'compile', 'run', 'execute', 'npm', 'docker']):
    ...
    def _categorize_query(self, query: str) -> Dict[str, Any]:
        ...
        agent_patterns = {
            'keywords': ['agent', 'agents', 'reviewer', 'reviewers', 'sub-agent', 'task delegation', 'delegate'],
            'actions': ['create', 'list', 'status', 'how many', 'count', 'delegate', 'terminate', 'manage']
        }
        ...
        tool_patterns = {
            'keywords': ['tool', 'tools', 'command', 'commands', 'capability', 'capabilities', 'function', 'functions'],
            'triggers': ['available', 'can use', 'list', 'what', 'show me', 'help', 'how to', 'which']
        }
        
        if any(kw in query_lower for kw in tool_patterns['keywords']) and any(trigger in query_lower for trigger in tool_patterns['triggers']):
            return {
        ...
        file_confidence = 0.0
        file_tools = []
        file_indicators = ['file', 'files', 'directory', 'folder', '.py', '.js', '.ts', '.md', '.txt', '.json', '.yaml', '.yml']
        
        # Boost confidence if file-related terms are present
        has_file_context = any(indicator in query_lower for indicator in file_indicators)
        ...
        git_patterns = {
            'keywords': ['git', 'commit', 'branch', 'merge', 'diff', 'repository', 'repo'],
        ...
        has_test_focus = any(kw in query_lower for kw in ['test', 'tests', 'testing']) and any(action in query_lower for action in ['run', 'execute'])
        
        if (git_match or git_action or status_match or diff_match) and not has_test_focus:
            git_tools = []
            ...
            if any(word in query_lower for word in ['diff', 'changes', 'difference']) or diff_match:
        ...
        testing_patterns = {
            'keywords': ['test', 'tests', 'testing', 'coverage', 'lint', 'quality', 'validate'],
            'frameworks': ['pytest', 'jest', 'unittest', 'mocha', 'go test', 'rspec'],
            'actions': ['run', 'execute', 'check', 'measure', 'report'],
            'quality_terms': ['code quality', 'static analysis', 'quality issues', 'code check'],
            'test_commands': ['run tests', 'execute tests', 'run test', 'execute test']
        }
        
        test_match = any(kw in query_lower for kw in testing_patterns['keywords'] + testing_patterns['frameworks'])
        test_action = any(action in query_lower for action in testing_patterns['actions'])
        quality_match = any(term in query_lower for term in testing_patterns['quality_terms'])
        test_command = any(cmd in query_lower for cmd in testing_patterns['test_commands'])
        ...
        architecture_patterns = {
            'keywords': ['architecture', 'structure', 'design', 'dependencies', 'patterns', 'analyze codebase', 
                        'code structure', 'project structure', 'overview', 'diagram', 'visualization'],
            'actions': ['analyze', 'review', 'examine', 'assess', 'evaluate', 'generate']
        }
        
        arch_match = any(kw in query_lower for kw in architecture_patterns['keywords'])
        arch_action = any(action in query_lower for action in architecture_patterns['actions'])
        ...
        explicit_shell = any(kw in query_lower for kw in shell_patterns['explicit_keywords'])
        command_indicator = any(cmd in query_lower for cmd in shell_patterns['command_indicators'])
        script_match = any(kw in query_lower for kw in shell_patterns['script_keywords'])
        ...
        memory_patterns = {
            'keywords': ['remember', 'save', 'recall', 'memory', 'session', 'context', 'persist', 'store'],
            'actions': ['save', 'load', 'remember', 'recall', 'store', 'retrieve']
        }
        
        # Memory clearing/destructive operations
        lobotomize_patterns = ['lobotomize', 'clear memory', 'forget everything', 'wipe memory', 'reset memory', 'start fresh', 'amnesia']
        
        memory_match = any(kw in query_lower for kw in memory_patterns['keywords'])
        memory_action = any(action in query_lower for action in memory_patterns['actions'])
        lobotomize_match = any(pattern in query_lower for pattern in lobotomize_patterns)
        ...
        # Reasoning/analysis
        reasoning_patterns = {
            'keywords': ['think', 'analyze', 'reasoning', 'decision', 'pros and cons', 'brainstorm', 
                        'problem solving', 'root cause', 'evaluate', 'assess'],
            'triggers': ['help me think', 'what are the', 'should i', 'how to approach', 'break down']
        }
        
        reasoning_match = any(kw in query_lower for kw in reasoning_patterns['keywords'])
        reasoning_trigger = any(trigger in query_lower for trigger in reasoning_patterns['triggers'])
        ...
        is_todo_search = any(term in query_lower for term in ['find todo', 'search todo', 'todo comments', 'all todo'])
        
        annotation_match = any(kw in query_lower for kw in annotation_patterns['keywords']) or any(term in query_lower for term in annotation_patterns['todo_creation'])
        annotation_action = any(action in query_lower for action in annotation_patterns['actions'])
        
        # Special case for "mark" + code/section which is annotation
        mark_code = 'mark' in query_lower and any(term in query_lower for term in ['code', 'section', 'function', 'class', 'line', 'important'])
        ...
        notebook_action = any(action in query_lower for action in notebook_patterns['actions'])
        ...
        for workflow_type, keywords in complex_patterns.items():
            ...
            if workflow_type == 'setup' and explicit_match:
                ...
                has_project_context = any(term in query_lower for term in ['project', 'package', 'environment', 'new'])
    ...
    async def build_context(self, query: str, max_files: int = 20) -> ProjectContext:
        ...
        if context_strategy == "none":
            ...
            max_files = min(max_files, 3)  # Minimal context (e.g., agent management, git ops)
        ...
        files_to_read = list(valid_analyses.keys())[:max_files * 2]  # Read more than needed for filtering
        ...
        for file_path, content in zip(files_to_read, contents):
            if isinstance(content, str) and content:
                ...
                if len(self.file_cache) < self.max_cache_size:
                    self.file_cache[file_path] = (content, valid_analyses[file_path].modified_time)
                else:
                    self._manage_cache_size()
                    self.file_cache[file_path] = (content, valid_analyses[file_path].modified_time)
        ...
        filtered_files = {f: content for f, content in file_contents.items() if f in relevant_files}
        filtered_info = {f: info for f, info in valid_analyses.items() if f in relevant_files}
